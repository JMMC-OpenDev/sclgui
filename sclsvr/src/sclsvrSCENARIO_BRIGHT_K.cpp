...
//      _requestI280.SetSearchBand("K");
    ...
    // I/280
    if (AddEntry(vobsCATALOG_ASCC_ID, &_requestI280, NULL, &_starListP, vobsCOPY, NULL, NULL, "&SpType=%5bOBAFGKM%5d*&e_Plx=%3E0.0&Plx=%3E0.999") == mcsFAILURE)
    {
        return mcsFAILURE;
    }
    ...
    // BORDE
    if (AddEntry(vobsCATALOG_LBSI_ID, &_request, NULL, &_starListP, vobsMERGE) == mcsFAILURE)
    {
        return mcsFAILURE;
    } 
    
    // MERAND
    if (AddEntry(vobsCATALOG_MERAND_ID, &_request, NULL, &_starListP, vobsMERGE) == mcsFAILURE)
    {
        return mcsFAILURE;
    } 
