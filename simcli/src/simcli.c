...
typedef struct
{ /* Data kept as result from Simbad Query */
...
int handle; /* IN: Associated handle number */
{
    ...
    data->handle = handle;
    data->abuf = DATABUF;
    data->buf = MALLOC(char, data->abuf);
    data->aline = DATALINE;
    data->line = MALLOC(int, data->aline);
    data->atype = DATATYPE;
    data->type = MALLOC(int, data->atype);
    data->aquery = 64;
    data->query = MALLOC(char, data->aquery);
    data->atell = MAXITYPE * 16;
    data->tell = MALLOC(char, data->atell);
    ClearData(data);
    return (data);
}
...
char *str; /* IN: Data made of lines, 1st word of each = datatype */
{
    ...
    for (b = data->buf + data->obuf; ; b = p)
    {
        ...
        *(p++) = 0; /* Replace the \n */
        ...
        if (data->uline >= data->aline)
        { /* Have to expand line array */
            data->aline += DATALINE;
...
int number; /* IN: Number in range 1...n */
{
    ...
    int *t;
    if (*data->query)
    { /* There is a query to perform. Do it ! */
