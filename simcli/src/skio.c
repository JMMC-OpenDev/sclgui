...
#define logfile stderr   /* Log-file for errors	*/
#endif
#include "simcliPrivate.h"

/*typedef void (*FCT)();*/
typedef struct
{ /* A buffer to save what's read */
...
static void logBuffer(char *fct, char *buf, int len)
/*++++++++++++++++
...
-----------------*/
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        switch (c = buf[i])
        {
            case '\\': fprintf(iologfile, "\\\\");
                continue;
            case '"': fprintf(iologfile, "\\\"");
                continue;
            case '\n': fprintf(iologfile, "\\n");
                continue;
            case '\r': fprintf(iologfile, "\\r");
                continue;
            case '\t': fprintf(iologfile, "\\t");
                continue;
            default:
                if (isprint(c)) fputc(c, iologfile);
                else fprintf(iologfile, "\\%03o", (uint) (c & 0xff));
                continue;
        }
    }
...
static void logValue(char *fct, int val)
/*++++++++++++++++
.PURPOSE  Log what's comes in
.RETURNS  0
-----------------*/
...
int len; /* IN: Max len to read */
{
    ...
    if (iologfile) logBuffer("sk_read(from saved buffer)", buf, bytes);
    return (bytes);
}
...
int len; /* IN: Length of data 	*/
{
    ...
    if (iologfile) logBuffer("sk_iosave", buf, len);
    if ((b = inputs[fno]))
...
int len; /* IN: Max len to read */
{
    ...
    if (iologfile) logBuffer("sk_read", buf, i);
    return (i);
}
...
int len; /* IN: Max len to read */
{
    ...
    if (iologfile) logBuffer("sk_get", buf, p - buf);
    return (p - buf);
}
...
int len; /* IN: Max len to read */
{
    ...
    if (iologfile) logBuffer("sk_gets", buf, p - buf);
    return (p - buf);
}
...
int fno; /* IN: opened socket */
{
    ...
    if (iologfile) logValue("sk_getl", ntohl(stat));
    return (ntohl(stat));
}
...
int len; /* IN: Max len to read */
{
    ...
    if (iologfile) logBuffer("sk_write", buf, p - buf);
    return (p - buf);
}
...
int val; /* IN: Value to write */
{
    ...
    if (iologfile) logValue("sk_putl", val);
    nval = htonl(val);
...
int fno; /* IN: socket to close */
{
    ...
    if (iologfile) logValue("sk_close", fno);
    return (stat);
}
