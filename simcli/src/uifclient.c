...
extern int sk_disconnect _PARAMS((int plug) );
...
*getstruct;
{
    ...
    /* LBO: clear head buffer */
    memset(head_string, 0, HEAD_BUF);
    ...
    length = atoi(&(head_string[3]));
    getstruct->length = length;
    ...
    if (sk_get(ear, getstruct->msg, length) == ERROR)
    {
        return ( CONNEXIONERROR);
    }
    getstruct->msg[length] = '\0';
