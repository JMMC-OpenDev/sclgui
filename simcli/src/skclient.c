...
{
    int ret;
    if (plug < 0) return (ERROR);
    if (plug >= NSOCKS) return (ERROR);
 ...
 * LBO: free resources
 */
int sk_disconnect(plug)
int plug; /* IN: Started Socket Number 	*/
{
    if (plug < 0 || plug > NSOCKS)
    {
        return (ERROR);
    }
...
char *service; /* IN: Service to connect to */
{
    ...
    int stat;
#endif
    struct timeval tv = { 15, 0 }; /* 15 seconds */
    ...
    {
        sprintf(buf, "%d", plug);
        err2("****sk_open: Too many opened sockets", buf);
        sk_close(plug);
        return (ERROR);
    }
    if (plugs[plug].server)
    ...
    {
        put_err("****sk_open: Can't setsockopt(TCP_NODELAY)");
        sk_close(plug);
        return (ERROR);
    }
#endif
    /* set timeout on socket connection */
    if (setsockopt(plug, SOL_SOCKET, SO_SNDTIMEO,
                   (struct timeval *)&tv, sizeof(struct timeval)) < 0)
    {
        put_err("****sk_open: Can't setsockopt(SO_SNDTIMEO)");
        sk_close(plug);
        return (ERROR);
    }
    
    /* set timeout on socket transfers */
    tv.tv_sec = 30;
    if (setsockopt(plug, SOL_SOCKET, SO_RCVTIMEO,
                   (struct timeval *)&tv, sizeof(struct timeval)) < 0)
    {
        put_err("****sk_open: Can't setsockopt(SO_RCVTIMEO)");
        sk_close(plug);
        return (ERROR);
    }
    ...
    /* Note: this is the MOST frequent error:
            ****sk_open: can't connect: Operation now in progress (29 times)
            ****sk_open: can't connect: Connection refused        (27 times)
     */
