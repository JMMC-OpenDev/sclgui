...
static alxSTAR_POPULATION *alxGetStarPopulation(void)
{
    ...
    {mcsFALSE, "alxStarPopulationInKBand.cfg",
        {0, 10, 90, 180, 270, 350},
    ...
    char *fileName;
    fileName = miscLocateFile(starPopulation.fileName);
    if (fileName == NULL)
    {
        return NULL;
    }
    ...
    while ((pos = miscDynBufGetNextLine(&dynBuf, pos, line, mcsTRUE)) != NULL)
    {
        ...
        if (strlen(line) != 0)
        {
            ...
            mcsINT32 nbMaxSubStrings = 
                1 + alxNB_GLON_STEPS * alxNB_GLAT_STEPS;
                                     ...
                                     mcsINT32           *nbOfStars)
{
    ...
    logDebug("Magnitude [%.1f - %.1f], range used => [%.1f - %.1f]",
             minMag, maxMag,
             starPopulation->mag[minMagIdx], starPopulation->mag[maxMagIdx]);
